// Pure chain-building logic for grouping a question's re-practice attempts in
// History: an isolated, dependency-free, unit-tested function (see
// `re-practice-chains.test.ts`) with no React / React Native / Supabase
// imports. Nothing here renders anything.
//
// A re-practice recording carries `recordings.re_practice_of` pointing at the
// recording its 3-dot menu was opened from. Because a freshly-assigned
// daily-pool question is retired the instant it's assigned, the ONLY way two
// recordings relate to the same question is a deliberate re-practice — so
// following the `re_practice_of` links (directly or transitively) to a root
// fully groups a question's attempts, with no need to also group by
// `question_id`.

// The minimal shape `buildChains` needs off a `recordings` row. `RecordingRow`
// is assignable to this; extra fields are carried through untouched on the
// `members` it returns.
...
export type RePracticeChain<T extends ChainRecording> = {
   ...
   * flag for the group card's star.
   */
  rootId: string;
  /**
 ...
 * NOT in the input list (the original was deleted — the FK is
 * `on delete set null`, so the value is usually already `null` — or a
 * paginated/filtered list simply doesn't contain the parent), starts a fresh
 * chain of its own. It is never a crash and never silently dropped.
 *
 ...
 * star reads from and toggles. For a single-member chain that's just the
 * recording itself.
 *
 * Used by History's favorites-only filter, so "a chain is favorited" means
 * precisely what the star on its card shows. (The chain *detail* screen's
 * header star uses a slightly richer rule — the largest sub-chain's root — to
 * stay sensible for a branched chain whose root was deleted; for a linear
 * chain the two agree, and the List only ever has the root's flag anyway.)
 */
 ...
 * that carries `question` text, else the literal `'No prompt'` (re-practice
 * requires a question, so the fallback shouldn't be hit). Shared by the
 * grouped History List card and the chain detail screen's header so the two
 * never show a different heading for the same group.
 */
