...
export async function fetchRecordingById(id: string): Promise<RecordingDetail | null> {
  const { data, error } = await supabase
    .from('recordings')
    .select(
      'id, mode, question, question_id, status, created_at, title, transcript, feedback, impact_score, clarity_score, structure_score, grammar_issue_count, audio_path, audio_deleted, favorite'
    )
    .eq('id', id)
    .maybeSingle();
  if (error) {
