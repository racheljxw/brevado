...
**v3 (Phase 6) — scoring + the Streaks tab — is now starting.** Scope is narrowed to two things:
three 0–100 per-recording scores (Impact / Clarity / Structure) generated by the *existing* Gemini
feedback call extended to return them, and the Streaks tab (client-side score aggregation). See
[v3 scope](#v3-scope) below for the dense summary. **Epic F Step 1** (schema + scoring generation +
the recording-detail-screen display swap) is the first step. **v4** — re-practice mode, the dynamic
question pool, additional modes — was split back out of v3 and is **deliberately not part of this
phase**; don't start building it.
...
**Streaks tab (Epic G).** Fills the empty placeholder tab (`src/app/(tabs)/streaks.tsx`) added in
v2. Daily-average score trends, streak counting, and trend-% calculation are all computed
